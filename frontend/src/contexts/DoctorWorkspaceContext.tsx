...
import { isIndependentDoctorTenant } from '../utils/tenantType';
...
export interface DoctorWorkspaceContextValue {
  ...
  /** True when `tenant_type` is `independent_doctor` (self-managed practice). */
  isIndependent: boolean;
  /**
   * When true, the UI should not offer create/edit workflows for care or billing
   * (hospital or clinic — server also enforces; this mirrors that for doctors).
   */
  isReadOnly: boolean;
...
export function DoctorWorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const isIndependent = isIndependentDoctorTenant(selfDoctor?.tenant_type);
  const profilePartial = !loading && !error && selfDoctor === null;
  const isReadOnly = !isIndependent;
