...
def _hospital_tenant_payload_hash(tenant_in: TenantCreate) -> str:
    body = tenant_in.model_dump(mode="json")
    canonical = json.dumps(body, sort_keys=True, separators=(",", ":"), default=str)
    return hashlib.sha256(canonical.encode("utf-8")).hexdigest()


def create_hospital_tenant(
    db: Session,
    ...
    if tenant_in.type != TenantType.hospital:
        raise ValidationError("Only hospital tenants can be created via this endpoint")

    if idempotency_key is not None:
        idempotency_key = idempotency_key.strip() or None

    body_hash = _hospital_tenant_payload_hash(tenant_in)
    if idempotency_key:
    ...
    # Auth deps may have begun a transaction; do not use begin_nested. Single commit at end.
    ...
    try:
        ...
        if idempotency_key:
            ...
            if existing is not None and existing.request_hash == body_hash:
                tenant2 = db.get(Tenant, existing.tenant_id)
                if tenant2 is None:
    ...
    logger.info(
        "[HOSPITAL CREATED] tenant_id=%s admin_user_id=%s",
        tenant.id,
        admin_user_id,
    )
