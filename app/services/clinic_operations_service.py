...
InventoryItem, InventoryStock, PurchaseOrder, etc.) to produce operational
...
def get_today_summary(db: Session, tenant_id: UUID) -> TodaySummaryCard:
    ...
    # Procurement pending (draft purchase orders)
    procurement_pending = (
...
# ═════════════════════════════════════════════════════════════════════════════
# 9. PROCUREMENT PENDING
# ═════════════════════════════════════════════════════════════════════════════


def get_procurement_pending(db: Session, tenant_id: UUID) -> list[ProcurementPendingItem]:
    """Purchase orders awaiting action (draft status)."""
    now = _utc_now()
    ...
    items: list[ProcurementPendingItem] = []
    for po in pos:
        supplier_name = ""
        if po.supplier_id:
        ...
        days_pending = 0
        if po.created_at:
            days_pending = (now - po.created_at).days
        ...
        items.append(
            ProcurementPendingItem(
                ...
                item_count=len(po.items) if hasattr(po, "items") and po.items else 0,
...
def get_clinic_operations_dashboard(
    ...
    procurement_pending = get_procurement_pending(db, tenant_id)
...
def get_operational_tasks(db: Session, tenant_id: UUID) -> OperationalTaskList:
    ...
    # ── Procurement pending tasks (medium priority) ──
    pending_pos = (
        db.query(PurchaseOrder)
        .filter(
            PurchaseOrder.tenant_id == tenant_id,
    ...
    for po in pending_pos:
        ...
        tasks.append(
            OperationalTaskItem(
                id=f"procurement-{po.id}",
                category=TaskCategory.procurement,
                priority=TaskPriority.medium,
                title=f"Procurement pending — {supplier_name or 'Unknown'}",
                description=f"PO status: {po.status.value if hasattr(po.status, 'value') else po.status}",
                entity_id=po.id,
                action_label="Review PO",
                action_url="/admin/procurement",
                created_at=po.created_at,
            )
        )
