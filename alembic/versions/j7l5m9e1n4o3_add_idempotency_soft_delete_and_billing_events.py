...
from migration_helpers import pg_column_exists, pg_constraint_exists, pg_index_exists, pg_table_exists
...
def upgrade():
    ...
    if not pg_column_exists('billings', 'idempotency_key'):
        op.add_column(
            'billings',
            sa.Column('idempotency_key', sa.String(length=255), nullable=True)
        )
    if not pg_constraint_exists('uq_billing_idempotency_key', table_name='billings'):
        op.create_unique_constraint('uq_billing_idempotency_key', 'billings', ['idempotency_key'])

    # Add is_deleted to billings
    if not pg_column_exists('billings', 'is_deleted'):
        op.add_column(
            'billings',
            sa.Column('is_deleted', sa.Boolean(), nullable=False, server_default='false')
        )
    if not pg_index_exists('idx_billing_is_deleted'):
        op.create_index('idx_billing_is_deleted', 'billings', ['is_deleted'])

    # Create billing_events table
    if not pg_table_exists('billing_events'):
        op.create_table(
            'billing_events',
